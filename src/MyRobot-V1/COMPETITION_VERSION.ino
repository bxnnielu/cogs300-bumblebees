...
// ------------------ Wall-follow tuning ------------------
const float WALL_SET_POINT = 17.0;   // desired right-wall distance (cm)
...
// ------------------ Thresholds ------------------
const int WALL_DETECTED_CM   = 45;  // side wall exists if <= 60 cm
const int FRONT_THRESHOLD_CM = 20;   // trigger LEFT_PIVOT if front <= 20 cm
const int FRONT_STOP_CM      = 20;   // in LEFT_PIVOT, stop when front <= 15 cm

// ------------------ Timed calibration ------------------
// tune these on your robot
const int FORWARD_2CM_MS    = 160;   // approx time to move forward 2 cm
const int PIVOT_LEFT_90_MS  = 450;   // approx left pivot 90 deg
const int PIVOT_RIGHT_90_MS = 450;   // approx right pivot 90 deg
...
long readFilteredUltrasonic(int trig, int echo) {

  long a = readUltrasonicCM(trig, echo);
  delay(5);
  long b = readUltrasonicCM(trig, echo);
  delay(5);
  long c = readUltrasonicCM(trig, echo);
...
void mazeController() {
  ...
  sideCm  = readFilteredUltrasonic(trigSide, echoSide);
  frontCm = readFilteredUltrasonic(trigFront, echoFront);
  ...
  switch (wfState) {
    ...
    case WF_FOLLOW:
      ...
      // CASE 1 — Right turn available
      if (rightOpen) {
        wfState = WF_RIGHT_PIVOT;
        return;
      }

      // CASE 2 — Dead end / left corner
      if (!frontOpen) {
...
void loop() {
    ...
    case ROBOT_MAZE:
      ...
      if (wallFollowMode) {
        ...
        if (frontCm > 200 && frontCm < 300) {
          ...
          Serial.println("MAZE COMPLETE -> OBJECT DETECTION");
        }
  ...
  if (millis() - lastPrint > 200) {
    ...
    if (wfState == WF_FOLLOW) Serial.print("FOLLOW");
    else if (wfState == WF_LEFT_PIVOT) Serial.print("LEFT_PIVOT");
    else if (wfState == WF_RIGHT_PIVOT) Serial.print("RIGHT_PIVOT");

    Serial.print(" | ROBOT STATE: ");

    if (robotState == ROBOT_MAZE) Serial.print("MAZE");
    else Serial.print("NEXT_ACTIVITY");
